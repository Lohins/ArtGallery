//
//  AGArtistLoginViewController.swift
//  ArtGallery
//
...
class AGArtistLoginViewController: UIViewController {
    ...
    func setupUI(){
        ...
        artistLogo.image = UIImage.init(named: "main_artist_icon")
        ...
        // email input
        self.emailBox = AGHasIconTextbox.init(frame: CGRect.init(x: 0, y: 0, width: boxWidth, height: boxHeight), iconName: "login_email_icon", text: String.localizedString("AGArtistLoginViewController-EmailAddress"), isSecureText: false)
        ...
        // prefill default email
        if let defaultemail = UserDefaults.standard.string(forKey: "artist_email"){
            self.emailBox.textField.text = defaultemail
        }

        // pwd input
        self.pwdBox = AGHasIconTextbox.init(frame: CGRect.init(x: 0, y: 0, width: boxWidth, height: boxHeight), iconName: "login_pwd_icon", text: String.localizedString("AGArtistLoginViewController-Password"), isSecureText: true)
        ...
        startButton.setTitle(String.localizedString("AGArtistLoginViewController-GetStarted"), for: .normal)
        ...
        createAccountLabel.text = String.localizedString("AGArtistLoginViewController-CreateAccount")
        ...
        forgetPwdLabel.text = String.localizedString("AGArtistLoginViewController-ForgetPassword")
    ...
    func LanguageStandardization(){
        ...
        titleLabel.text = String.localizedString("AGArtistLoginViewController-NavTitle")
    ...
    func loginAction(_ sender: AnyObject) {
        ...
        loginService.ArtistLogin(email: email, pass: pwd) { [weak self](flag, error) in
            if let weakSelf = self{
                if flag == false {
                ...
                // 没有偏好设置的去设置
                if artist.info.tagList == [] && artist.info.subjectList == []{
    ...
    @IBAction func createAccountAction(_ sender: AnyObject) {
        ...
        let createVC = AGArtistCreateViewController()
        self.navigationController?.pushViewController(createVC, animated: true)
    }
    
    
    @IBAction func retrieveAction(_ sender: AnyObject) {
